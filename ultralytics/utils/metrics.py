...
def snr_distance(embeddings):
    """
    Calculate the SNR distance matrix for a set of embeddings.

    Parameters:
    embeddings (np.ndarray): Array of shape (num_samples, embedding_dim) representing the embeddings.

    Returns:
    np.ndarray: Distance matrix of shape (num_samples, num_samples) containing the SNR distances.
    """
    # Step 1: Compute variances for each embedding (1D array of shape (num_samples,))
    anchor_variances = np.var(embeddings, axis=1)

    # Step 2: Compute the mean and variance of all embeddings
    num_samples, embedding_dim = embeddings.shape

    # Compute pairwise differences and variances efficiently
    pairwise_variances = np.zeros((num_samples, num_samples))
    for i in range(num_samples):
        diff = embeddings - embeddings[i]  # Only compute the difference for row i
        pairwise_variances[i] = np.var(diff, axis=1)

    # Step 3: Compute the SNR distance matrix
    distance_matrix = pairwise_variances / anchor_variances[:, np.newaxis]
